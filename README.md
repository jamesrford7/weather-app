...
* The dto utility file, while heavily under test, reeks of code-smells and brittleness. I've tried to break things out to illustrate intention of the functions, but I'd refactor this if I had the chance. There is also inefficient iterating through the object that comes back from the API.
* I'd have liked to make the dropdown populate from the API, rather than config, although I'd imagine an horrifically long drop-down would be the result! So I've kept it to some of our major cities, should keep enough people happy :D
